  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('http://localhost:8080/api/v1/vinci', {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        ...
        await response.json();
        alert('Image has been successfully shared with community');
        navigate('/');
      } catch (err) {
        alert(err);
      } finally {
  ...
  return (
    <section className="max-w-8xl mx-auto flex flex-col justify-center items-center bg-[#F5F5F5] rounded-lg py-5 px-5">
      ...
      <div  className='flex items-center flex-col'>
        <h1 className="font-extrabold text-[#222328] text-[2.5rem] bg-gradient-to-r from-teal-500 via-purple-500 to-orange-500 bg-clip-text text-transparent background-animate text-shadow">Generate</h1>
      ...
      <form className="mt-16 w-full" onSubmit={handleSubmit}>
        ...
        <div className='flex justify-center items-center mb-10 w-full'>
          <button
            ...
            className="w-full flex justify-center items-center text-white bg-green-700 font-medium rounded-md text-sm sm:w-auto px-20 py-2.5 text-center"
          >
